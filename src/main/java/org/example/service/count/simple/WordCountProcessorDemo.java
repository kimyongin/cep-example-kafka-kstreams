...
public class WordCountProcessorDemo {
    ...
    public WordCountProcessorDemo(Topology topology) {
        new WordCountProcessorDemo(topology, true);
    }
    public WordCountProcessorDemo(Topology topology, boolean withRepartition) {
        if (withRepartition) {
            buildTopology(topology);
        } else {
            buildTopologyWithoutRepartition(topology);
        }
    }

    public void buildTopology(Topology topology) {
        StoreBuilder<KeyValueStore<String, Long>> storeBuilder = Stores.keyValueStoreBuilder(Stores.inMemoryKeyValueStore("processor-count"), STRING_SERDE, LONG_SERDE);
    ...
    public void buildTopologyWithoutRepartition(Topology topology) {
        StoreBuilder<KeyValueStore<String, Long>> storeBuilder = Stores.keyValueStoreBuilder(Stores.inMemoryKeyValueStore("processor-count"), STRING_SERDE, LONG_SERDE);

        topology.addSource("source", "streams-app-processor-input");
        topology.addProcessor("split", WordSplitProcessor::new, "source");
        topology.addProcessor("count", () -> new WordCountProcessor("processor-count"), "split");
        topology.addStateStore(storeBuilder, "count");
        topology.addProcessor("filter", FilterProcessor::new, "count");
        topology.addSink("sink", "streams-app-processor-output", STRING_SERDE.serializer(), LONG_SERDE.serializer(), "filter");
    }
